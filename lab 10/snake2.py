...
def login():
    ...
    if user:
        ...
        result = cur.fetchone()
        current_level = result[0] if result[0] is not None else 0
        print(f"Welcome, {username}! Current level: {current_level}")
        conn.close()
        return user_id, current_level
    else:
        print(" Invalid login. Exiting game.")
        conn.close()
        return None, None
        ...
        for event in pygame.event.get():  # Обработка событий
            ...
            if event.type == pygame.KEYDOWN:  # Если пользователь нажимает клавишу
                if event.key == pygame.K_p:
                    ...
                    text = font.render("Paused. Game saved.", True, font_colour)
                    display.blit(text, (50, 250))
